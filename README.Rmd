...
This package is a wrapper for the Sarim package (also available via github).
Note that for Sarim C++11 must be enabled. This can be done in R with:
```{r, eval = FALSE}
# install Sarim:
...
The SOIR() function can be used within Sarim.
This is a short example how to use:
```{r, eval = FALSE}
library(Sarim)
library(SOIR)
library(magrittr)
